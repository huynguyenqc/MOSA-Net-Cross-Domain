...
* Step 0: Preparation
  ...
  * Download data from VoiceMOS challenge
  * Create a conda environment and install Python 3.6
* Step 1: Feature extraction
  * Run `git submodule update --init --recursive` to clone the submodule of
  `fairseq` package (in `./fairseq-lib`)
  * Run `cd fairseq-lib`
  * Add the following lines to the end of the class `HubertConfig` in file `fairseq-lib/fairseq/models/hubert/hubert.py`
  ```python
  ...
  * Run `cd ..`
  * Run `mkdir fairseq`
  * Download Hubert Large model (<a href="https://github.com/pytorch/fairseq/tree/main/examples/hubert#load-a-pretrained-model" target="_blank">hubert_large_ll60k.pt</a>) and save in
  `./fairseq`
  * Run `Extracting_Hubert_Feature_VoiceMOS_Challenge.py`
  * *Note: There is one big file in validation set. You may have to remove it.*
* Step 2: Model training
  * Run `pip install nvidia-pyindex`
  * Run `pip install nvidia-tensorflow`
  * Modify `SincNet.py` as follows
  ```python
  ...
  ```
  * Modify `MOSA-Net_Cross_Domain_VoiceMOS_Challeneg.py` as follows
  ```python
  """
  ...
  def train(Train_list, Train_list_hubert, Num_train, Test_list, Test_list_hubert, Num_testdata, pathmodel, track_name):
      ...
      with tf.Session() as sess:
          ...
          hist=model.fit_generator(
              g1,steps_per_epoch=Num_train, epochs=epoch, verbose=1,validation_data=g2,validation_steps=Num_testdata,max_queue_size=1, workers=1,callbacks=[checkpointer])
  ...
  ```
  * Run `TF_KERAS=1 python MOSA-Net_Cross_Domain_VoiceMOS_Challenge.py`
