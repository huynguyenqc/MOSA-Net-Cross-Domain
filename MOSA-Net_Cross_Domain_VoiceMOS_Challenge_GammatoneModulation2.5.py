...
def Sp_and_phase(path, Noisy=False):
    ...
    if not os.path.exists(
            f'./data/phase1-main/gammatonemodulation_512/{basename}.npz'):
        ...
        data = np.load(f'./data/phase1-main/gammatonemodulation_512/{basename}.npz')
...
def train(Train_list, Train_list_hubert, Num_train, Test_list, Test_list_hubert,
    ...
    model.compile(loss={'MOS_score': 'mse', 'Frame_score': 'mse'},
                  optimizer=adam)
    plot_model(model, to_file='model_' + str(model_name) + '_GammatoneTimeModulation25_epoch_' + str(
    ...
    if track_name == 'phase1-main':
        ...
        model.load_weights(
            './PreTrained_VoiceMOSChallenge/MOSA-Net_Cross_Domain-main_GammatoneTimeModulation25_epoch_100.h5')

    with open(pathmodel + '_GammatoneTimeModulation25_epoch_' + str(epoch) + '.json',
              'w') as f:  # save the model
        f.write(model.to_json())
    checkpointer = ModelCheckpoint(
        filepath=pathmodel + '_GammatoneTimeModulation25_epoch_' + str(epoch) + '.hdf5', verbose=1,
    ...
    with tf.Session() as sess:
        ...
        # new_model.save(pathmodel + '_GammatoneTimeModulation25_epoch_' + str(epoch) + '.h5')
        hist = model.fit_generator(
            g1, steps_per_epoch=Num_train, epochs=epoch, verbose=1,
            validation_data=g2, validation_steps=Num_testdata, max_queue_size=1,
            workers=1, callbacks=[checkpointer])
        model.save(pathmodel + '_GammatoneTimeModulation25_epoch_' + str(epoch) + '.h5')
    ...
    plt.show()
    plt.savefig(
        'Learning_curve_' + str(model_name) + '_GammatoneTimeModulation25_epoch_' + str(epoch) + '.png',
        dpi=150)
...
def Test(Test_List, Test_List_Hubert_feat, pathmodel, track_name):
    ...
    with tf.Session() as sess:
        model_test.summary()
        sess.run(tf.global_variables_initializer())
        # model_test.load_weights(pathmodel + '_GammatoneTimeModulation25_epoch_' + str(epoch) + '.h5')
        # model_test.load_weights(
        #     './PreTrained_VoiceMOSChallenge/MOSA-Net_Cross_Domain-main_GammatoneTimeModulation25_epoch_100.h5')
        model_test.load_weights(
            './PreTrained_VoiceMOSChallenge/MOSA-Net_Cross_Domain-main_GammatoneTimeModulation25_epoch_100.hdf5')
    ...
    with open('List_predicted_score_mos' + str(model_name) + '_GammatoneTimeModulation25_epoch_' + str(
    ...
    plt.savefig(
        ('Scatter_plot_MOS_' + str(model_name) +
         '_GammatoneTimeModulation25_epoch_' + str(epoch) + '.png'),
        dpi=150)
