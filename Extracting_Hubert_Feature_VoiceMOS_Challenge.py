...
def Extract_SSL_Feat(filepath, model, list_new,dirname, track_name):
    ...
    path = './data/'+track_name+'/DATAtest/DATA/wav/'+name
...
def Save_NPY_Test(Test_data, track_name):
    cp_path = './fairseq/hubert_large_ll60k.pt'
    model, cfg, task = fairseq.checkpoint_utils.load_model_ensemble_and_task([cp_path], arg_overrides={"data": "./fairseq/data/dict"})
    model = model[0]
    model.eval()
    ...
    dirname='./data/'+track_name+'/Hubert/test/'
    if not os.path.exists(dirname):
        os.makedirs(dirname)
        print('Creating Directory')       
    ...
    for index in range(len(Test_data)):   
        pesq_filepath = Test_data[index].split(',')
        list_new=Extract_SSL_Feat(pesq_filepath, model, list_new,dirname, track_name)    

    with open('./data/'+track_name+'/DATAtest/DATA/sets/List_Npy_Test_hubert_MOS_Challenge_phase1_main.txt','w') as g:
        for item in list_new:
          g.write("%s\n" % item)
...
if __name__ == '__main__':
    ...
    Save_NPY_Test(Test_list, track_name)
